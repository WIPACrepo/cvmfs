...
class PyDask(PythonPackage):
    ...
    depends_on('py-cloudpickle@1.1.1:',        type=('build', 'run'))
    depends_on('py-fsspec@0.6.0:',             type=('build', 'run'))
    ...
    depends_on('py-scipy',      type=('build', 'run'), when='+dataframe')
    ...
    depends_on('py-toolz@0.8.2:',       type=('build', 'run'), when='+delayed')

    # Requirements for dask.distributed
    depends_on('py-distributed',        type=('build', 'run'), when='+distributed')

    @property
    def import_modules(self):
        modules = [
            'dask', 'dask.bytes', 'dask.diagnostics',
        ]
