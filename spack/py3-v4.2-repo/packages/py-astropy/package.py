...
class PyAstropy(PythonPackage):
    ...
    variant('extras', default=True, description='Enable extra functionality')

    # Required dependencies
    depends_on('python@3.7:', when='@4.3:', type=('build', 'run'))
    ...
    depends_on('py-numpy@1.17:', when='@4.3:', type=('build', 'run'))
    ...
    depends_on('py-pyerfa', when='@4.3:', type=('build', 'run'))
    ...
    depends_on('py-extension-helpers', when='@4.0:', type=('build', 'run'))
    depends_on('py-jinja2', when='@4.0:', type=('build','run'))
    depends_on('py-certifi', when='@4.0:', type=('build','run'))
    ...
    def set_build_deps(self):
        if self.spec.version >= Version('3.0.0'):
            lib_dir = self.spec['wcslib'].prefix.lib
    ...
    def setup_environment(self, spack_env, run_env):
        if self.version >= Version('4.0'):
            spack_env.set('ASTROPY_USE_SYSTEM_WCSLIB', '1')
            spack_env.set('ASTROPY_USE_SYSTEM_CFITSIO', '1')
            spack_env.set('ASTROPY_USE_SYSTEM_EXPAT', '1')
