...
class Gnuplot(AutotoolsPackage):
    ...
    version("6.0.4", sha256="458d94769625e73d5f6232500f49cbadcb2b183380d43d2266a0f9701aeb9c5b")
    ...
    def disable_docs(self):
        if "~doc" in self.spec:
            # Gnuplot's generated Makefile unconditionally includes 'docs' in its SUBDIRS.
            # This regex safely removes 'docs' so the build step skips the directory.
            filter_file(r"^(SUBDIRS\s*=.*)\bdocs\b(.*)", r"\1\2", "Makefile")
            
            # (Optional) You can also skip the 'demo' directory to speed up the build further
            filter_file(r"^(SUBDIRS\s*=.*)\bdemo\b(.*)", r"\1\2", "Makefile")
